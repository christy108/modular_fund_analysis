...
Usage:
    python -m pipeline.dashboard base_none                      # serve on :8080
    python -m pipeline.dashboard base_none esg_refinitiv        # compare two configs
    python -m pipeline.dashboard base_none --port 5000
    python -m pipeline.dashboard base_none --markdown           # text only, no Taipy
...
``pipeline.run.run``, so every run is still archived to ``runs/``) and hands the
...
from pipeline.run import run as run_config
