# `pipeline/` — the auditable fund-analysis pipeline
...
`python -m pipeline.registry` prints `validate.ok`, the topological order and the external
...
```bash
python -m pipeline.dashboard base_none                   # runs it, serves on :8080
python -m pipeline.dashboard base_none esg_refinitiv     # two configs side by side
python -m pipeline.dashboard base_none --port 5000
python -m pipeline.dashboard base_none --markdown        # text only, no Taipy needed
```
...
manifests created in the current process. The driver reuses `pipeline.run.run()`, so those
...
```bash
python -m pipeline.boundary     # self-test: proves each round trip is an identity
```
...
Every `python -m pipeline.run <config>` writes to **two** places
([run.py](run.py)):
...
4. `python -m pipeline.registry` — validate before you run anything.
...
`python -m pipeline.run <name>` and appears in the dashboard with no other changes.
...
- **`pipeline_new/` is not this folder.** It is an untracked experimental copy with 10 of
  the 12 nodes. `pipeline/` is canonical — it is what `run.py`, `parity/` and `tests/`
  import.
